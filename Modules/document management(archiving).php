...
<body>
    ...
    <script>
        ...
        function loadCategoryFiles(category) {
            ...
            // Special handling for Inventory - Placeholder for API integration
            if (category === 'Inventory') {
                const grid = document.getElementById(gridId);
                grid.innerHTML = '<div style="text-align: center; padding: 3rem; color: #666; grid-column: 1/-1;"><p>⏳ Loading inventory records...</p></div>';

                /**
                 * API INTEGRATION POINT FOR INVENTORY
                 * 1. Change the URL below to your Inventory API endpoint (e.g., ../integ/inventory_api.php)
                 * 2. Ensure the API returns a JSON array of objects
                 * 3. Database structure should correspond to the headers: Item Name, Category, Stock Level, Location, Last Updated
                 */
                const INVENTORY_API_URL = '../integ/inventory_fn.php'; // REPLACE WITH ACTUAL API PATH

                const renderInventoryTable = (data) => {
                    if (!data || data.length === 0) {
                        grid.innerHTML = '<div style="text-align: center; padding: 3rem; color: #adb5bd; grid-column: 1/-1;"><p>No inventory records found.</p></div>';
                        return;
                    }
                                    ...
                                    ${data.map(item => `
                                        <tr>
                                            ...
                                            <td style="text-align: center;">
                                                <span style="padding: 2px 8px; border-radius: 4px; background: ${(item.stock || item.quantity || 0) < 10 ? '#fff5f5' : '#f0fdf4'}; color: ${(item.stock || item.quantity || 0) < 10 ? '#c53030' : '#22c55e'}; font-weight: bold;">
                                                    ${item.stock || item.quantity || 0}
                                                </span>
                                            </td>
                                            <td>${item.location || 'Warehouse'}</td>
                                            <td style="text-align: center;">${new Date(item.updated_at || item.upload_date || Date.now()).toLocaleDateString()}</td>
                                            <td style="text-align: center;">
                                                <button class="btn-view-small" onclick='alert("Viewing item: " + "${item.name || item.item_name}")'>
                                                    <i class="fas fa-eye"></i> View
                                                </button>
                                            </td>
                                        </tr>
                ...
                const sampleInventory = [
                    { name: 'Bed Linens (King)', category: 'Housekeeping', stock: 45, location: 'Store A', upload_date: new Date() },
                    { name: 'Bath Towels', category: 'Housekeeping', stock: 120, location: 'Store A', upload_date: new Date() },
                    { name: 'Mini Bar Sodas', category: 'F&B', stock: 8, location: 'Kitchen', upload_date: new Date() }
                ];
                renderInventoryTable(sampleInventory);
                
                /* 
                // FUTURE INTEGRATION: Uncomment this and connect to your database API
                fetch(INVENTORY_API_URL)
                    .then(response => response.json())
                    .then(data => {
                         if(data && data.length > 0) renderInventoryTable(data);
                    })
