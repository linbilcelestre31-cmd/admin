...
function generateReport() {
    ...
    reportData += `<div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 25px; border-bottom: 2px solid var(--gray-100); padding-bottom: 15px; flex-wrap: wrap; gap: 10px;">
                    ...
                    <button onclick="downloadReportPDF()" class="btn-primary-action" style="width: auto; padding: 8px 16px; font-size: 0.85rem; background: #3b82f6;">
...
function downloadReportPDF() {
    const reportContent = document.getElementById('report-data').innerHTML;
    if (!reportContent) {
        ...
        <html>
            <head>
                ...
                <style>
                    ...
                    .stat-card { flex: 1; border: 1px solid #e2e8f0; padding: 15px; border-radius: 10px; }
                    ...
                    .status-completed { background: #fff1f2; color: #9f1239; }
                    h3 { border-bottom: 2px solid #3b82f6; padding-bottom: 10px; }
                    button { display: none; } /* Hide view buttons in print */
                    .fa-download, h3 button { display: none !important; }
                </style>
            </head>
            <body>
                ${reportContent}
                <div style="margin-top: 50px; text-align: center; font-size: 12px; color: #94a3b8;">
                    Generated by Atiéra Admin System on ${new Date().toLocaleString()}
                </div>
            </body>
        </html>
    `);
    ...
    // Smooth delay to allow styles to load then print
    setTimeout(() => {
        printWindow.print();
        printWindow.close();
    }, 500);
