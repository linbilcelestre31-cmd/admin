<?php
/**
 * REFINED SSO RECEIVER for HR4 - Version 6.0
 */
session_start();
...
// 3. Fetch Secret Key (Check 'HR4' and handle fallback)
$stmt = $conn->prepare("SELECT secret_key FROM department_secrets WHERE department='HR4' AND is_active=1 LIMIT 1");
$stmt->execute();
$res = $stmt->get_result()->fetch_assoc();

// Use the key from DB or the standard pattern as fallback
$secret = ($res) ? trim($res['secret_key']) : "hr4_secret_key_2026";
